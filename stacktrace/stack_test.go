...
import (
	...
	"dev.gaijin.net/go/golib/stacktrace"
)
...
func TestStack(t *testing.T) {
	...
	t.Run("AddCaller", func(t *testing.T) {
		...
		assert.Equal(t, "dev.gaijin.net/go/golib/stacktrace_test.takeOddCallers", frames[0].Function)
		assert.True(t, strings.HasSuffix(frames[0].File, "/stacktrace/stack_test.go"))
		assert.Equal(t, 23, frames[0].Line)

		assert.Equal(t, "dev.gaijin.net/go/golib/stacktrace_test.TestStack.func1", frames[1].Function)
	...
	t.Run("String", func(t *testing.T) {
		...
		assert.Equal(t, "dev.gaijin.net/go/golib/stacktrace_test.takeOddCallers", lines[0])
		assert.Equal(t, "dev.gaijin.net/go/golib/stacktrace_test.TestStack.func2", lines[2])
